...
class EveSARG(Eve):
    ...
    def eavesdropStates(self, states: list[tuple[int, int]]) -> None:
        ...
        self.logger.log(f'Eve eavsdrops on Alice possible states')
    ...
    def eavsdropUsedStates(self, usedStates: list[int]) -> None:
        ...
        self.logger.log(f'Eve eavsdrops on Alice possible States')
        self.logger.log(
            f'Eve got {len(self.keyBits)} bits of key, out of which she knows only {len([bit for bit in self.keyBits if bit >= 0])}')
