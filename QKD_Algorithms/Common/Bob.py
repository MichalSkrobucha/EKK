...
class Bob:
    ...
    def receive(self):
        ...
        self.logger.log(f"Bob read {len(impulse)} photons from channel")
        ...
        self.logger.log(f"Bob measured {clickCount} clicks")
        ...
        if clickCount == 0:
            ...
            for (base, bit) in [(0, 0), (0, 1), (1, 0), (1, 1)]:
                if measurments[base][bit] == 1:
                    ...
                    self.logger.log(f"Bob's base is {base} and bit is {bit}")
                    return
        else:
            ...
            if base >= 0:
                ...
                if measurments[base][0] == 0:
                    self.bases.append(base)
                    self.bits.append(1)
                    self.logger.log(f"Bob's base is {base} and bit is {1}")
                    return
                elif measurments[base][1] == 0:
                    self.bases.append(base)
                    self.bits.append(0)
                    self.logger.log(f"Bob's base is {base} and bit is {0}")
                    return
    ...
    def sendSampleIds(self) -> list[int]:
        ...
        self.logger.log("Bob is sending to Alice indexes of sample bits")
    ...
    def receiveSamples(self, sample: list[int]) -> None:
        ...
        self.logger.log(f"Bob is recieving samples from Alice {sample}")
